...
| **ISS** | The station on a dark world map with its ground track for the next 90 minutes, the day/night terminator, and live altitude and speed. Computed in your browser, no polling. |
...
- **Frontend:** React 19, TypeScript, Vite, Tailwind v4, TanStack Query, React Router, Recharts,
  react-leaflet, framer-motion, satellite.js.
...
(plus username and password), and `CORS_ORIGINS` (your web origin). Set on the frontend:
`VITE_API_BASE_URL` pointing at the backend. In-memory caching is used in production, so there is no
Redis to run.
